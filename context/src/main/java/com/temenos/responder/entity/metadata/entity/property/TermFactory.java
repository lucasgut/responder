package com.temenos.responder.entity.metadata.entity.property;
...
public class TermFactory {
	...
	public String getTermDefaultValue(String name) {
		if(name != null) {
			if(name.equalsIgnoreCase(TermMandatory.TERM_NAME) ||
					name.equalsIgnoreCase(TermComplexType.TERM_NAME) ||
					name.equalsIgnoreCase(TermIdField.TERM_NAME)) {
				return "false";
			} else if (name.equalsIgnoreCase(TermRestriction.TERM_NAME)) {
				return TermRestriction.Restriction.NORESTRICTION.getValue();
			}
		}
		return null;
	}
