...
class CustomerInformation extends AbstractFlow {
    ...
    public void doExecute(ExecutionContext executionContext) {
        try {
            ...
            responseBody.set(ScaffoldCustomer.CUSTOMER_ID.name, extnCustomer.get(ScaffoldExternalCustomer.CUSTOMER_ID.name));
            responseBody.set(ScaffoldCustomer.CUSTOMER_NAME.name, extnCustomer.get(ScaffoldExternalCustomer.CUSTOMER_NAME.name));
            responseBody.set(ScaffoldCustomer.CUSTOMER_ADDRESS.name, extnCustomer.get(ScaffoldExternalCustomer.CUSTOMER_ADDRESS.name));
