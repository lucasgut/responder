...
class T24CustomerInformationTest extends Specification {
    ...
    @Unroll
    def "Set 'finalResult' attribute to #map if customer ID '#id' is requested"(id, map) {
        setup:
    ...
    @Unroll
    def "Set 'finalResult' attribute to an empty entity if nonexistent customer ID '#id' is requested"(id) {
        setup:
            def command = new T24CustomerInformation()
            def context = Mock(CommandContext)
        when:
